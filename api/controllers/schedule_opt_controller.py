...
def sch_opt_task(file_path: str , cache_key, check_if_processing_key:str):
        ...
        states =  [{"messages" :  json.dumps(task), "task_id": task.get("task_id"), "mode": "default" ,"task_type": "proj_sum", "status": schopt_status } for task in tasks_list]
        ...
        if len(encoding.encode(json.dumps(mapped_results))) <= 30000:
            summary_state =  {"messages" : json.dumps(mapped_results)  + f"\n num_tasks : {len(mapped_results)}", "mode": "default" , "task_type": "proj_sum", "status": schopt_status}
            ...
            chunk_summary_state =  [{"messages" : json.dumps(results) + f"\n num_tasks : {len(results)}" , "mode": "default" , "task_type": "proj_sum", "status": schopt_status} for results in tasks_results]
        ...
        final_summary =  schedule_opt_service([summary_state])[0] 
        schopt_status = cache.get(check_if_processing_key)
        schopt_status =  sch_opt_status(**schopt_status)
        ...
        if not hasattr(final_summary.get("messages")[-1] , "error"):
           save_tasks(other_tasks = ["delay_analysis", "sch_opt" , "risk_forecast" , "project_report"] ,cache_key =cache_key , state = final_summary , report_dict =  schopt_status.to_dict())
