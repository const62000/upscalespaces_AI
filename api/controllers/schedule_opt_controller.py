...
def sch_opt_task(file_path: str , cache_key):
        ...
        final_summary =  schedule_opt_service(summary_state) #pass summary_state to llm and get overall project delay analysis 
        end_time = time.time()
        elapsed_time = round((end_time - start_time)/60 , 2)  #in mins
        logging.warning(f"analysis completed [sch opt] , analysis took {elapsed_time} mins")
