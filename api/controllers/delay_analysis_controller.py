...
def delay_task(file_path: str , cache_key:str , check_if_processing_key:str):
        ...
        logging.warning(f"starting delay analysis for {len(tasks_list)} tasks")
...
def delay_analysis_controller(request):
    ...
    if file and file.name.endswith(('.xer')):  
        ...
        if cached_result:
            ...
            cache.set(check_if_processing_key , {'task_id': t.id, 'data_key': key , 'status': 'processing'} , timeout=60*60*10)
